...
The owner-facing **Refresh now** button runs the same bounded collector, then
reloads the validated selected workspace. A writable workspace first uses the
existing fail-closed reconciler; an observer or imported workspace skips that
authoritative write and scans only. It does not add a second collection path:
the collector lock still prevents overlap, disabled sources remain disabled,
and degraded sources remain visible.
