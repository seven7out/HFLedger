...
LAUNCHER_HTML = NATIVE / "src" / "index.html"
LAUNCHER_JS = NATIVE / "src" / "main.js"
...
class TextSizeSettingsTests(unittest.TestCase):
    def test_launcher_control_has_closed_accessible_choices_and_live_preview(self):
        markup = LAUNCHER_HTML.read_text(encoding="utf-8")
    ...
    def test_settings_menu_focuses_text_size_and_menu_names_match_product_language(self):
        ...
        self.assertIn('"app.settings" => show_launcher_settings(app)', source)
        self.assertIn('window.addEventListener("hfledger:show-settings"', launcher)
        self.assertIn('elements["pref-text-size"].focus', launcher)
    ...
    def test_clients_have_no_browser_local_text_size_copy(self):
        for path in (LAUNCHER_JS, SERVED_JS, DECK_JS):
        ...
        launcher = LAUNCHER_JS.read_text(encoding="utf-8")
        self.assertIn('invoke("update_preferences"', launcher)
        self.assertIn('elements["pref-text-size"].value = prior', launcher)
