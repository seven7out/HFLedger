"""Versioned public validator for the ``hfl-history-envelope`` contract, v1.

This is the production home of the contract proven fictionally at
``docs/redesign-v2/prototypes/effectiveness-history-adapter-v1/contract.py``.
The prototype file is preserved unchanged as review evidence; this module is
the one the adapter and tests target.

Two deliberate deltas against the prototype, both disclosed for review, both
replacing forced fabrication with declared absence:

1. ``lifecycleEvents[].runId`` may be null.  Retained ledger evidence events
   carry no source-native run identity; synthesizing a per-line run with an
   invented status is exactly the inference the source audit rejects.
2. ``blockerEpisodes[].runIds`` may be empty for the same reason.

Everything else is unchanged: the schema is closed, unknown versions and
fields fail closed, conflicting duplicate ids reject the envelope, coverage
windows for one source/scope must not overlap, and canonical output is
deterministically ordered regardless of input order.
"""
...
RETENTION_POLICIES = frozenset(("append-only", "rolling-window", "source-defined"))
RETENTION_STATES = frozenset((
    "complete", "left-truncated", "right-truncated", "both-truncated", "unknown",
))
DELETION_STATES = frozenset(("none-declared", "declared", "unknown"))
BACKFILL_STATES = frozenset(("none", "records-only"))
MAX_LATE_ARRIVAL_SECONDS = 90 * 86400


class HistoryContractError(ValueError):
    """A closed validation error for the public history contract."""
...
def validate_and_canonicalize(value):
    ...
    _enum(raw_bounds["retentionPolicy"], RETENTION_POLICIES,
          "historyBounds.retentionPolicy")
    _enum(raw_bounds["retentionState"], RETENTION_STATES,
          "historyBounds.retentionState")
    _enum(raw_bounds["deletionState"], DELETION_STATES,
          "historyBounds.deletionState")
    _enum(raw_bounds["backfillState"], BACKFILL_STATES,
    ...
    if (not isinstance(late_window, int) or isinstance(late_window, bool)
            or not 0 <= late_window <= MAX_LATE_ARRIVAL_SECONDS):
    ...
    for event in records["lifecycleEvents"].values():
        ...
        if event.get("runId") is not None:
            _require_refs([event["runId"]], records["runs"], label)
    ...
    for transition in records["reviewTransitions"].values():
        ...
        if transition.get("runId") is not None:
            _require_refs([transition["runId"]], records["runs"], label)
    ...
    for blocker in records["blockerEpisodes"].values():
        ...
        if closed is not None and closed < opened:
            raise HistoryContractError(f"{label} closes before it opens")
        run_ids = _id_list(blocker.get("runIds"), f"{label}.runIds")
